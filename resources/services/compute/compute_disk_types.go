...
func ResolveDiskTypeId(_ context.Context, _ schema.ClientMeta, resource *schema.Resource, c schema.Column) error {
	...
	if disk.Id != 0 {
		return helpers.WrapError(resource.Set(c.Name, cast.ToString(disk.Id)))
	}
	linkParts := strings.Split(disk.SelfLink, "/")
	return helpers.WrapError(resource.Set(c.Name, fmt.Sprintf("%s/%s", linkParts[len(linkParts)-3], disk.Name)))
}
