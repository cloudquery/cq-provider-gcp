...
func BigqueryDatasetAccesses() *schema.Table {
	return &schema.Table{
		...
		Columns: []schema.Column{
			...
			{
				Name:        "domain",
				Description: "[Pick one] A domain to grant access to Any users signed in with the domain specified will be granted the specified access Example: \"examplecom\" Maps to IAM policy member \"domain:DOMAIN\"",
			...
			{
				Name:        "user_by_email",
				Description: "[Pick one] An email address of a user to grant access to For example: fred@examplecom Maps to IAM policy member \"user:EMAIL\" or \"serviceAccount:EMAIL\"",
				Type:        schema.TypeString,
			},
			{
